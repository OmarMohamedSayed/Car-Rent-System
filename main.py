...
conn = MySQLdb.connect(host='db',user='root',passwd = 'root',port = 3306,db="car_rent_system") #enter Database username and password of mysql here
cursor = conn.cursor()
master_password = "ADMIN"  #master password for Delete any admin
custid = ""
driverID = 0
CARID = ""
payment_type = ""
...
#		encrypted_SQans = obj.encrypt(security_ans)					#Encrypting security Question Answer
		...
		# msg=Message("Welcome to RENTING CARS SERVICE",sender="Car Rentel Services",recipients=[email])
		# msg.body = "Thank you for signing up forRENTING CARS SERVICE, Now Book a Cab Now !!!"
		# mail.send(msg)
		'''except Exception, e:
			return(str(e)) '''	
...
def credit_payment():
	...
	km = {'Cairo-Alex': 200,'Cairo-Giza':165,'Cairo-ghardaqa':680,'Cairo-AinSokhna':150,'Cairo-Ismailia':160}
	cursor = conn.cursor()
...
def statusdriver():
	...
	tbooking = tbook[0][0]
	cursor.execute("""SELECT COUNT(*) FROM Booking WHERE cab_route = 'Cairo-Alex' """)
	NP = cursor.fetchall()
	NProute = int(NP[0][0])
	cursor.execute("""SELECT COUNT(*) FROM Booking WHERE cab_route = 'Cairo-ghardaqa' """)
	NN = cursor.fetchall()
	NNroute = int(NN[0][0])
	cursor.execute("""SELECT COUNT(*) FROM Booking WHERE cab_route = 'Cairo-Giza' """)
	NM = cursor.fetchall()
	NMroute = int(NM[0][0])
	cursor.execute("""SELECT COUNT(*) FROM Booking WHERE cab_route = 'Cairo-Ismailia' """)
	NA = cursor.fetchall()
	NAroute = int(NA[0][0])
	cursor.execute("""SELECT COUNT(*) FROM Booking WHERE cab_route = 'Cairo-AinSokhna' """)
	ND = cursor.fetchall()
	NDroute = int(ND[0][0])
	if NProute > NNroute and NProute > NMroute and NProute > NAroute and NProute > NDroute:
		most_used_route = "Cairo-Alex"
	elif NNroute > NProute and NNroute > NMroute and NNroute > NAroute and NNroute > NDroute:
		most_used_route = "Cairo-ghardaqa"
	elif NMroute > NNroute and NMroute > NProute and NMroute > NAroute and NMroute > NDroute:
		most_used_route = "Cairo-Giza"
	elif NAroute > NNroute and NAroute > NMroute and NAroute > NProute and NAroute > NDroute:
		most_used_route = "Cairo-Ismailia"
	elif NDroute > NNroute and NDroute > NMroute and NDroute > NAroute and NDroute > NProute:
		most_used_route = "Cairo-AinSokhna"
